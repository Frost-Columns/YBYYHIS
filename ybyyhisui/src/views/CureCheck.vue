      ...
      <el-header>
        <el-row class="row-bg">
          <el-col :md="1" style="margin: 11px;">
            <span style='color: white ; font-size: 16px'>检查管理</span>
          </el-col>
        </el-row>
      </el-header>
      <el-container>
        <el-aside width="400px">
          <el-container>
            ...
            <el-main style="margin: -20px 0 0 0;">
              <div>
                <el-tag style="width: 100%">未检查</el-tag>
                <el-table
                    :data="curelogslist600"
                    stripe
              ...
              <div>
                <el-tag type="success" style="width: 100%">检查中</el-tag>
                <el-table
                    :data="curelogslist601"
                    stripe
                    highlight-current-row
                    :row-style="{height: '53px'}"
                    height="220"
              ...
              <div>
                <el-tag type="warning" style="width: 100%">已检查</el-tag>
                <el-table
                    :data="curelogslist602"
                    stripe
                    highlight-current-row
                    :row-style="{height: '53px'}"
                    height="220"
...
export default {
  ...
  methods: {
    getCureLogsList() {
      ...
      axios.post("/getCheckCureList", params).then((res) => {
        this.curelogslist600 = res.data.data.curelogslist600
        this.curelogslist601 = res.data.data.curelogslist601
        this.curelogslist602 = res.data.data.curelogslist602
      })
    },
    qy(row) {
      var params = new URLSearchParams();
      params.append("clid", row.clid)
      axios.post("/CheckQy", params).then((res) => {
    ...
    onSubmit() {
      var params = new URLSearchParams();
      params.append("clid", this.curelogsinfo.clid)
      axios.post("/CheckWc", params).then((res) => {
        this.getCureLogsList()
      })
    },
